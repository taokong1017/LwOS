...
APP_PARTITION_DEFINE(app2);

#define ROOT_TASK_NAME "Root_Task"
APP_DATA(app2) task_id_t root_task_id = -1;
APP_DATA(app2) struct task root_task = {0};
APP_BSS(app2) char root_stack[TASK_STACK_SIZE] = {0};

#define TASK_A_NAME "Task_A"
APP_DATA(app2) task_id_t task_a_id = -1;
APP_DATA(app2) struct task task_a = {0};
APP_BSS(app2) char task_a_stack[TASK_STACK_SIZE] = {0};

#define TASK_B_NAME "Task_B"
APP_DATA(app2) task_id_t task_b_id = -1;
APP_DATA(app2) struct task task_b = {0};
APP_BSS(app2) char task_b_stack[TASK_STACK_SIZE] = {0};
...
static void task_a_entry(void *arg0, void *arg1, void *arg2, void *arg3) {
	...
	forever() {
		...
		user_task_delay(1);
		user_task_resume(task_b_id);
		user_task_delay(1);
		user_task_stop(task_b_id);
		user_task_delay(1);
...
int main() {
	app_mem_domain_init(&app_mem_domain, "app2", app2);
