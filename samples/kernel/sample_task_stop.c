...
static void test_task4_entry(void *arg0, void *arg1, void *arg2, void *arg3) {
	...
	for (;;) {
		...
		printf("%s has stop other tasks\n", TEST_TASK4_NAME);
		task_delay(10);
		task_start(test_task1_id);
		task_start(test_task2_id);
		task_start(test_task3_id);
		printf("%s has started other tasks\n", TEST_TASK4_NAME);
		task_delay(20);
	}
...
static void create_test_task4() {
	...
	ret = task_priority_set(test_task4_id, TASK_PRIORITY_HIGHEST);
	if (ret != OK) {
		printf("set task %s priority failed\n", task_name);
		return;
	}

	ret = task_cpu_affi_set(test_task4_id, 3);
	if (ret != OK) {
		printf("set task %s cpu affinity failed\n", task_name);
		return;
	}
